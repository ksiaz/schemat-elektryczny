...
export function ZugL2Node({ data, selected }: NodeProps<ZugL2NodeType>) {
  return (
    <div className={`flex flex-col items-center ${selected ? 'ring-2 ring-blue-500' : ''}`} style={{ width: 30 }}>
      <Handle type="source" position={Position.Top} id="in" className="!w-1.5 !h-1.5" style={{ backgroundColor: WIRE_COLORS.L2, left: 10 }} />
      <svg width="30" height="30" viewBox="0 0 30 30">
        <rect x="5" y="5" width="20" height="20" fill="none" stroke={WIRE_COLORS.L2} strokeWidth="1.5" />
        <text x="15" y="18" textAnchor="middle" fontSize="9" fill={WIRE_COLORS.L2} fontWeight="bold">{data.label}</text>
      ...
      <Handle type="source" position={Position.Bottom} id="out" className="!w-1.5 !h-1.5" style={{ backgroundColor: WIRE_COLORS.L2, left: 10 }} />
    </div>
  );
}
