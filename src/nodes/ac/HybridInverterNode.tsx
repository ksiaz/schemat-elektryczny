...
// Falownik hybrydowy — wszystkie handle na stalych px (wielokrotnosci 10)
export function HybridInverterNode({ data, selected }: NodeProps<HybridInverterNodeType>) {
  return (
    <div className={`flex flex-col items-center ${selected ? 'ring-2 ring-blue-500' : ''}`} style={{ width: 200 }}>
      ...
      {/* BAT +/- lewy bok */}
      <Handle type="target" position={Position.Left} id="bat-plus" className="!w-1.5 !h-1.5" style={{ backgroundColor: WIRE_COLORS.DC, top: 40 }} />
      <Handle type="target" position={Position.Left} id="bat-minus" className="!w-1.5 !h-1.5" style={{ backgroundColor: WIRE_COLORS.N, top: 60 }} />

      <svg width="200" height="150" viewBox="0 0 130 95">
      ...
      {/* COM prawy bok */}
      <Handle type="source" position={Position.Right} id="com" className="!w-1.5 !h-1.5" style={{ backgroundColor: '#999', top: 40 }} />
      {/* PE obudowy lewy bok */}
