...
// Magazyn energii — DC+, DC-, COM, PE obudowy
export function BatteryNode({ data, selected }: NodeProps<BatteryNodeType>) {
  return (
    <div className={`flex flex-col items-center ${selected ? 'ring-2 ring-blue-500' : ''}`}>
      ...
      <Handle type="source" position={Position.Top} id="dc-plus" className="!w-2 !h-2" style={{ backgroundColor: WIRE_COLORS.DC, left: '25%' }} />
      <Handle type="source" position={Position.Top} id="dc-minus" className="!w-2 !h-2" style={{ backgroundColor: WIRE_COLORS.N, left: '50%' }} />
      {/* COM — komunikacja */}
      <Handle type="source" position={Position.Top} id="com" className="!w-2 !h-2" style={{ backgroundColor: '#999', left: '75%' }} />
      ...
      {data.parameters.capacity && (
        <div className="text-[10px] text-gray-500">{String(data.parameters.capacity)} kWh</div>
      )}

      {/* PE obudowy — dol */}
      <Handle type="source" position={Position.Bottom} id="pe-housing" className="!w-2 !h-2" style={{ backgroundColor: WIRE_COLORS.PE }} />
    </div>
  );
}
