...
export function CableRouteNode({ data, selected }: NodeProps<CableRouteNodeType>) {
  ...
  return (
    <div className={`flex flex-col items-center ${selected ? 'ring-2 ring-blue-500 ring-offset-2' : ''}`}>
      ...
      <Handle type="target" position={Position.Left} id="in-left" className="!w-2 !h-2" style={{ backgroundColor: color }} />
      ...
      <Handle type="source" position={Position.Bottom} id="out" className="!w-2 !h-2" style={{ backgroundColor: color }} />
      <Handle type="source" position={Position.Right} id="out-right" className="!w-2 !h-2" style={{ backgroundColor: color }} />
    </div>
  );
}
