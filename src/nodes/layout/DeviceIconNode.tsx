...
const DEVICE_ICONS: Record<string, { svg: string; color: string }> = {
  inverter: {
    svg: '<rect x="5" y="5" width="30" height="30" fill="white" stroke="black" stroke-width="1.5"/><text x="20" y="22" text-anchor="middle" font-size="8" font-family="monospace">=~</text>',
    color: '#333',
  },
  enclosure: {
  ...
  charger: {
    svg: '<rect x="5" y="5" width="30" height="30" fill="white" stroke="black" stroke-width="1.5" rx="3"/><text x="20" y="24" text-anchor="middle" font-size="8" font-weight="bold">EV</text>',
    color: '#333',
  },
};
...
export function DeviceIconNode({ data, selected }: NodeProps<DeviceIconNodeType>) {
  ...
  return (
    <div className={`flex flex-col items-center ${selected ? 'ring-2 ring-blue-500 ring-offset-2' : ''}`}>
      <Handle type="target" position={Position.Top} id="in" className="!bg-gray-500 !w-2 !h-2" />
      ...
      <Handle type="source" position={Position.Bottom} id="out" className="!bg-gray-500 !w-2 !h-2" />
    </div>
  );
}
