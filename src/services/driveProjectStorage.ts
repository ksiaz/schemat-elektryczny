import { ensureToken, invalidateToken } from './googleAuth.ts'
...
export class DriveProjectStorage implements ProjectStorage {
  ...
  private async findFileByProjectId(projectId: string): Promise<DriveFile | null> {
    ...
    const r = await this.request(`${API}/files?q=${q}&fields=files(id,name,modifiedTime,appProperties)&spaces=drive`)
  ...
  async list(): Promise<ProjectMeta[]> {
    ...
    const r = await this.request(`${API}/files?q=${q}&fields=files(id,name,modifiedTime,appProperties)&orderBy=modifiedTime desc&spaces=drive`)
    if (!r.ok) throw new Error(`Drive list: ${r.status}`)
    const j = await r.json() as { files: DriveFile[] }
  ...
  async load(id: string): Promise<ProjectFile> {
    ...
    const r = await this.request(`${API}/files/${f.id}?alt=media`)
  ...
  async create(name: string, data: ProjectData): Promise<ProjectMeta> {
    ...
    const r = await this.request(`${UPLOAD}/files?uploadType=multipart&fields=id,modifiedTime`, {
      method: 'POST', headers: { 'Content-Type': 'multipart/related; boundary=BOUND' }, body,
    })
  ...
  async save(id: string, name: string, data: ProjectData, knownUpdatedAt?: string): Promise<SaveResult> {
    ...
    const body = this.multipart(metadata, data)
    const r = await this.request(`${UPLOAD}/files/${existing.id}?uploadType=multipart&fields=id,modifiedTime`, {
      method: 'PATCH', headers: { 'Content-Type': 'multipart/related; boundary=BOUND' }, body,
    })
  ...
  async rename(id: string, name: string): Promise<void> {
    ...
    const r = await this.request(`${API}/files/${f.id}`, {
  ...
  async remove(id: string): Promise<void> {
    const f = await this.findFileByProjectId(id)
    if (!f) return
    const r = await this.request(`${API}/files/${f.id}`, { method: 'DELETE' })
